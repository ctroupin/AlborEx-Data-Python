...
\newcommand{\ctroupin}{\textcolor{blue}}
\begin{document}


\title{The AlborEx dataset: Multi-scale, physical properties of the Alboran Sea during the AlborEx mission (25-30 May 2014)}
...
\begin{abstract}
AlborEx (Alboran Sea Experiment) consisted of a multi-platform, multi-disciplinary experiment carried out in the Alboran Sea (Western Mediterranean Sea) between May 25 and 31, 2014. The observational component of AlborEx aimed to sample the physical and biogeochemical properties of oceanographic features present along an intense frontal zone, with a particular interest in the vertical motions in its vicinity. To this end, the mission included 1 research vessel (66 CTD profiles, ADCP, thermosalinograph), 2 underwater gliders (adding up 554 profiles), 3 profiling floats and 25 surface drifters. 

The CTD acquired profiles of temperature and conductivity (from which salinity was derived), oxygen concentration, chlorophyll concentration, turbidity, Photosynthetically Available Radiation and beam attenuation. Both the deep and the coastal gliders measured temperature,  conductivity and oxygen concentration and chlorophyll concentration. 
%AP 
\textbf{yo aquí también pondría la descripción de las variables medidas por el CTD y los gliders}
%CT 
\ctroupin{hecho}

%AP 
\textbf{añadir una frase del tipo: This article presents the AlborEx dataset and makes it available to the public} 
%CT 
\ctroupin{Hecho al final del abstract}
...
This goal of the article is to present the AlborEx dataset and to make it available to the public. The most recent version is available at \url{http://doi.org/10.5281/zenodo.1328238}.
...
The AlborEx multi-platform experiment was performed in the Alboran Sea from from May 25 to 31, 2014, with the objective of capturing meso and submesoscale processes and evaluating their specific contribution to the vertical motion at an intense oceanic front.

The observing system, described in the next section, is made up of the SOCIB coastal R/V, 2 underwater gliders, 3 profiling floats and 25 surface drifters. The resulting data set is particularly rich thanks due to the variety of sensors and measured variables concentrated on a relatively small area.
...
\caption{Sea surface temperature in the western Mediterranean Sea from MODIS sensor on-board Aqua satellite corresponding to May 25 and 30, 2014. The dashed black line indicates the approximate position of the front based on the temperature gradient for the period 25--30 May. Level-2, 11 $\mu m$, night-time images were selected. Only pixels with a quality flag equal to 1 (good data) were conserved and represented on the map. Note that the same front position is used in the subsequent figures.\label{fig2:SST}}
\end{figure}

\subsection{In situ data}

Whereas the remote sensing measurements helped in the mission design and the front detection, in situ observation were essential to fulfill the mission objectives. The different platforms used for the data collection are presented hereinafter.
...
The distinct water properties on both sides of the front are evidenced by the T-S diagrams in Fig.~\ref{fig4:TSdiag}, where the colors represent the fluorescence. The salinity range north of the front is roughly between 38 and 38.5, with the exception of a few measurements, and confirms the nature of the Mediterranean Water mass. 

%AP
\textbf{esto podría ser un poco redundante con Pascual et al. pero ya lo dirán los revisores, no?}
%CT 
\ctroupin{creo que es normal que algunas cosas salen en los 2 artículos. Aquí la dificultad es evitar hacer interpretación de los datos. Si es el caso, quitare frases.}

The fluorescence maximum appears between 14 and 15$^{\circ}$C. South of the front the salinity range is wider while the temperature values are similar to the north.
...
In addition to the CTDs, the R/V thermosalinograph continuously acquired temperature and conductivity along the ship track, from which near surface salinity is derived (Fig.~\ref{fig3:thermosal}).

%AP 
\textbf{entiendo que estos datos también formarán parte del dataset, no?}
\ctroupin{Sí, claro, de hecho ya salen en el link zenodo que puse. Al final del manuscript pondré una tabla con nombres de los ficheros disponibles y descripción de la plataforma, así será todo + explicito.}
The R/V weather station acquired air temperature, pressure, wind speed and direction during the whole duration of the mission. Direct measurements of currents were performed with acoustic Doppler current profiler and are presented in Sec.~\ref{sec:adcp}. 
%AP 
\textbf{y estos?}
\ctroupin{también estos datos, aunque no haré ningun plot de datos de meteo (viento a veces es util pero poca precisión).}
\begin{figure}[t]
...
The Vessel Mounted-Acoustic Doppler Current Meter Profiler (VM-ADCP) operating at 153 kHz acquired velocity profiles approximately every 2 minutes during nighttime (22:00--6:00 UTC) at a speed of 10 knots and during the CTD surveys (see Fig.~\ref{fig3:CTD}). The measurement accuracy is on the order of 0.01~m/s. The measurements were vertically averaged over 8~m depth bins.

The velocities exhibit a dominant eastward current with speed locally larger than 1~m/s. \ctroupin{Also needs to plot the position of the front and the velocity field at another depth.}

\begin{figure}[h]
\includegraphics[width=.475\textwidth]{fig09.png}
\caption{[Figure to be adapted]. Velocity field obtained with the ADCP at a 16 m depth. \label{fig9:adcp}}
\end{figure}
...
\item[\tt L1] is the processing level (see Sec.~\ref{sec:proclevels}).
...
\verb#dep0001_profiler-drifter-arvora3001_ogs-arvora3001_L1_2014-05-25.nc# & Arvor-A float			\\
...
\authorcontribution{C.T. prepared the figures and the first version of the manuscript. A.P. and S.R. edited the manuscript and helped with the .... J.G.F. lead the data management and the creation of a DOI in Datacite. C.M. implemented the processing of the ADCP data.}
